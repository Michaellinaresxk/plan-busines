<template>
  <div
    :class="['reservation-card', { 'is-priority': reservation.isPriority }]"
  ...
  >
    ...
    <div class="service-section">
      <div class="service-tags">
        ...
        <span v-if="reservation.isPriority" class="priority-tag">
          Priority
        </span>
      </div>
...
import IconPriority from './icons/IconPriority.vue';
...
// Service color mapping with a more sophisticated palette
...
const serviceColor = computed(() => {
  return serviceColorMap[props.reservation.service as keyof typeof serviceColorMap] || '#6366F1';
});

const avatarBackground = computed(() => {
  // Create a lighter version of the service color for the avatar
  const color = serviceColor.value;
  return `${color}22`; // Adding 22 for 13% opacity in hex
});
...
function formatDate(dateString: string): string {
  try {
    // Assuming date is in ISO format or a consistent format
